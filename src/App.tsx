...
import { BookOpen, Sparkles, Heart, RefreshCw, BookmarkCheck, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  const handleToggleSave = (book: BookRecommendation) => {
    const exists = savedBooks.some((b) => b.id === book.id || b.title === book.title);
    if (exists) {
      const updated = savedBooks.filter((b) => b.id !== book.id && b.title !== book.title);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-teal-200 selection:text-teal-900 flex flex-col">
          ...
          <div className="space-y-8 w-full max-w-full">
            ...
            {recommendations.length > 0 ? (
              <BookList
                ...
                onRefresh={() => currentRequest && handleFetchRecommendations(currentRequest)}
                isLoading={isLoading}
              />
            ) : (
      ...
      <footer className="bg-white border-t border-slate-200/80 py-6 mt-12 text-center text-xs text-slate-500">
        <div className="max-w-4xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <p>© {new Date().getFullYear()} 오늘의 책 추천 — 독자를 위한 품격 있는 AI 도서 큐레이터</p>
        </div>
      </footer>
